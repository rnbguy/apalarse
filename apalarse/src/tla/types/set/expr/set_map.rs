use crate::tla::{Context, Expr, Map, SetExpr, Variable};

#[derive(Debug, Clone)]
pub struct SetMap<S, P, U>
where
    S: SetExpr,
    P: Fn(Variable<S::ElemType>) -> U,
    U: Expr,
{
    pub set: S,
    pub predicate: P,
}

impl<S, P, U> SetMap<S, P, U>
where
    S: SetExpr,
    P: Fn(Variable<S::ElemType>) -> U,
    U: Expr,
{
    pub const fn new(set: S, predicate: P) -> Self {
        Self { set, predicate }
    }
}

impl<S, P, U> Expr for SetMap<S, P, U>
where
    S: SetExpr,
    P: Fn(Variable<S::ElemType>) -> U + Clone,
    U: Expr,
{
    type Output = Map<S::ElemType, U::Output>;

    fn tla_expr(&self, cx: &mut Context) -> String {
        cx.enter_scope();
        let new_var = cx.bound_var();
        let rt = format!(
            "[{} \\in {} |-> {}]",
            new_var.tla_expr(cx),
            self.set.tla_expr(cx),
            (self.predicate)(new_var.clone()).tla_expr(cx),
        );
        cx.exit_scope();
        rt
    }

    fn evaluate(&self) -> Self::Output {
        todo!()
    }
}
